...
let name : string = name ^ "Shih"

(* 1.c. Create a string containing your email address *)
let email : string = "brian@airkit.com"
...
let rec num_divisors n num =
    if num > n
      then 0 
      else if n mod num = 0 
        then 1 + num_divisors n (num + 1)
        else num_divisors n (num+1)
  in
  if  num_divisors n 1 < m then true else false
...
print "hellso %s\n" (string_of_int(num_divisors 4 1))
...
let hello: string = string_of_bool(few_divisors 17 3);; (* true -- 17 has only 1 and 17 *)
print "Year: %s\n" hello
 ...
 *)
 ...
 let rec sin_pi (num: int): float =
  if num = 0
    then 3.0
    else sin_pi (num - 1) +. sin(sin_pi(num - 1));;

print "sin %s" (string_of_float(sin_pi(3)))
...
let rec monte_pi n =
  if n = 0 
    then 0 
      ...
      let test a b =
        if a *. a +. b *. b <= 1.0 then true else false in
...
let hello = monte_pi 1000;;
print "sin %s" (string_of_int(hello))
